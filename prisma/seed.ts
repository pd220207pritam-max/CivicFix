...
const categories = [
  ...
  'Drainage Problem', 'Damaged Road', 'Traffic Signal',
  'Public Toilet', 'Illegal Dumping', 'Other'
]

const statuses = ['Submitted', 'Under Review', 'Assigned', 'In Progress', 'Resolved', 'Rejected']

// India-wide coordinates for demo data (20 different states/cities)
const locations = [
...
async function main() {
  console.log('🌱 Starting CivicFix seed...')
  ...
  // Keep users so admins don't get deleted, or delete users except admin?
  // Let's just delete dummy complaints. We deleted history, notifications, and complaints.
  ...
  const statusList: string[] = ['Submitted', 'Under Review', 'Assigned', 'In Progress', 'Resolved', 'Resolved']
  ...
  for (let i = 0; i < complaintData.length; i++) {
    ...
    // Add small offset to make locations unique
    ...
    // Add subsequent status history for more advanced statuses
    if (['Under Review', 'Assigned', 'In Progress', 'Resolved'].includes(status)) {
      const reviewDate = new Date(createdAt.getTime() + 2 * 60 * 60 * 1000)
      await prisma.statusHistory.create({
        data: {
          status: 'Under Review',
          note: 'Issue has been logged and is under review by our team.',
          complaintId: complaint.id,
    ...
    if (['Assigned', 'In Progress', 'Resolved'].includes(status) && dept) {
      const assignDate = new Date(createdAt.getTime() + 24 * 60 * 60 * 1000)
      await prisma.statusHistory.create({
        data: {
          status: 'Assigned',
          note: `Complaint assigned to ${dept.name}`,
          complaintId: complaint.id,
          changedById: admin.id,
    ...
    if (['In Progress', 'Resolved'].includes(status)) {
      const progressDate = new Date(createdAt.getTime() + 48 * 60 * 60 * 1000)
      await prisma.statusHistory.create({
        data: {
          status: 'In Progress',
          note: 'Work has started on this issue. Expected completion in 3-5 days.',
          complaintId: complaint.id,
          changedById: admin.id,
    ...
    if (status === 'Resolved') {
      const resolveDate = new Date(createdAt.getTime() + 5 * 24 * 60 * 60 * 1000)
      await prisma.statusHistory.create({
        data: {
          status: 'Resolved',
          note: 'Issue has been successfully resolved. Thank you for reporting!',
          complaintId: complaint.id,
          changedById: admin.id,
          createdAt: resolveDate,
        },
      })
      ...
      await prisma.notification.create({
        data: {
          ...
          message: `Your complaint "${data.title}" has been marked as resolved.`,
          type: 'success',
          userId: citizen.id,
        },
      })
  ...
  console.log(`✅ Created ${complaintData.length} demo complaints`)
  console.log('')
  console.log('🎉 Seed completed successfully!')
  console.log('')
  console.log('Demo credentials:')
  console.log('  Admin: admin@civicfix.com / admin123')
  console.log('  Citizen: citizen@civicfix.com / citizen123')
}
